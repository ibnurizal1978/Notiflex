...
if(!empty($query))
{
  ...
  while ($r = $query->fetch_array())
  {
    ...
    $nestedData['reminder_day']         = $r['reminder_day'];
    $nestedData['reminder_via']         = $r['reminder_via'];
    $nestedData['aksi'] 			    = '<a href=reminderDayDetail?'.Encryption::encode($r[$id]).' class="badge bg-green text-blue-fg text-uppercase">Set Reminder Day</a> <a href=reminderTypeDetail?'.Encryption::encode($r[$id]).' class="badge bg-blue text-blue-fg text-uppercase">Edit</a>';
    $data[] = $nestedData;
    $no++;
  }
}
