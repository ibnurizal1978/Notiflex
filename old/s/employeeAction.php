...
if($action == "add") {
  ...
  //sukses
  $s = "INSERT INTO $tbl SET name = '".$name."', position = '".$position."', phone = '".$phone."', email = '".$email."', type = 'EMPLOYEE', client_id = '".$_SESSION['client_id']."', created_at = UTC_TIMESTAMP()";
  mysqli_query($conn, $s);
  $last_id = mysqli_insert_id($conn);

  /* add log */
  addLog($conn, $_SESSION['user_id'],'ADD EMPLOYEE', 'Add new employee name: '.$name);
  echo "<script>";
