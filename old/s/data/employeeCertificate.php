...
if(empty($_POST['search']['value']))
{
  ...
  $sql = "SELECT * $godeg AND (full_name LIKE '%$search%' OR email LIKE '%$search%' OR phone LIKE '%$search%') order by $order $dir LIMIT $limit OFFSET $start";
  $query = $conn->query($sql);

 	$querycount = $conn->query("SELECT count(employee_id) as jumlah $godeg AND (full_name LIKE '%$search%' OR email LIKE '%$search%' OR phone LIKE '%$search%')");
	$datacount = $querycount->fetch_array();
