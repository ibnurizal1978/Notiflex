...
$query = "INSERT INTO tbl_user (username, password) VALUES ('admin', '".$hash."')";
*/

$txt_username	=	input_data(filter_var($_POST['txt_username'],FILTER_SANITIZE_STRING));
$txt_password	=	input_data($_POST['txt_password']);
...
if(password_verify($txt_password, $row['password']))
{
  ...
  $sql_nav_header = "SELECT nav_header_id,nav_header_icon,nav_header_name FROM tbl_nav_user a INNER JOIN tbl_nav_menu b using (nav_menu_id) INNER JOIN tbl_nav_header USING (nav_header_id) WHERE user_id = '".$row['user_id']."' AND nav_header_name <> 'Settings' GROUP BY nav_header_id ORDER by nav_header_name";
  ...
  /*$sql_nav_header2 = "SELECT nav_header_id,nav_header_icon,nav_header_name FROM tbl_nav_user a INNER JOIN tbl_nav_menu b using (nav_menu_id) INNER JOIN tbl_nav_header USING (nav_header_id) WHERE user_id = '".$row['user_id']."' AND nav_header_name = 'Settings' GROUP BY nav_header_id ORDER by nav_header_name";
  $h_nav_header2 = mysqli_query($conn,$sql_nav_header2);
  while($row_menu_header2 = mysqli_fetch_assoc($h_nav_header2)) {
  	$_SESSION['nav_header2'][]= array('header_id' => $row_menu_header2['nav_header_id'],'header_icon' => $row_menu_header2['nav_header_icon'],'header_name' => $row_menu_header2['nav_header_name']);

  	$sql_menu2 	= "SELECT nav_header_id,nav_menu_name, nav_menu_url FROM tbl_nav_user a INNER JOIN tbl_nav_menu b using (nav_menu_id) WHERE user_id = '".$row['user_id']."' AND nav_header_id = '".$row_menu_header2['nav_header_id']."' GROUP BY nav_menu_id ORDER by nav_menu_sort_order";
    $h_menu2 	= mysqli_query($conn,$sql_menu2);
  	while($row_menu2 = mysqli_fetch_assoc($h_menu2)) {
  		$_SESSION['nav_items2'][]= array('url' => $row_menu2['nav_menu_url'], 'name' => $row_menu2['nav_menu_name'],'nav_header_id' => $row_menu2['nav_header_id']);
  	}
  }*/

  //tampilkan menu berdasarkan level - ini yang pakai category
  /*
  $sql_nav_header2 = "SELECT category_id, category_name FROM tbl_nav_category_user a INNER JOIN tbl_category USING (category_id) WHERE user_id = '".$row['user_id']."' GROUP BY category_id ORDER by category_name";
  $h_nav_header2 = mysqli_query($conn,$sql_nav_header2);
  while($row_menu_header2 = mysqli_fetch_assoc($h_nav_header2)) {
    $_SESSION['nav_category_header'][]= array('category_id' => $row_menu_header2['category_id'],'category_name' => $row_menu_header2['category_name']);
  }*/
    
  //var_dump($_SESSION['nav_items2']);
  //exit();
