...
*/
$s = "SELECT a.list_id, a.category_id, a.list_expiration_id, category_name, date_format(start_date, '%M %d, %Y') as start_date, date_format(expired_date, '%M %d, %Y') as expired_date, date_format(expired_date, '%Y-%m-%d') as expired_date2, datediff(expired_date, CURDATE()) difference, c.name as held_by, e.name as name, photo_name FROM tbl_list_expiration a INNER JOIN tbl_category b USING (category_id)  INNER JOIN tbl_list c ON a.held_by = c.id INNER JOIN tbl_list e ON a.list_id = e.id INNER JOIN tbl_list_upload d ON a.list_expiration_id = d.list_expiration_id WHERE archived_status = 0 AND date(expired_date) >= curdate()";
...
if(mysqli_num_rows($h) == 0)
{
    ...
    {
        echo '<hr/><b>ID: </b> '.$r['list_expiration_id'].' | <b>Name: '.$r['name'].' | Expired in: '.$r['difference'].' | expired date: '.$r['expired_date'].' | cat: '.$r['category_name'].'</b><br/>';
        $s2 = "SELECT category_reminder_id, reminder_day, client_id FROM tbl_category_reminder WHERE category_id = '".$r['category_id']."'";
